...
def initialize_chain(test_dir):
    ...
    for i in range(4):
        ...
        initialize_datadir(test_dir, i) # Overwrite port/rpcport in credits.conf
...
def start_node(i, dir, extra_args=None, rpchost=None):
    """
        Start a creditsd and return RPC connection to it
        """
    datadir = os.path.join(dir, "node"+str(i))
    args = [ "creditsd", "-datadir="+datadir, "-keypool=1" ]
    if extra_args is not None: args.extend(extra_args)
    creditsd_processes.append(subprocess.Popen(args))
    devnull = open("/dev/null", "w+")
    subprocess.check_call([ "creditsd-cli", "-datadir="+datadir] +
...
def start_nodes(num_nodes, dir, extra_args=None, rpchost=None):
    """
        Start multiple creditsd, return RPC connections to them
        """
