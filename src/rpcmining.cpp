...
Value credits_estimatefee(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "estimatefee nblocks\n"
    ...
    CFeeRate feeRate = credits_mempool.estimateFee(nBlocks);
...
Value bitcoin_estimatefee(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "bitcoin_estimatefee nblocks\n"
            "\nEstimates the approximate fee per kilobyte\n"
            "needed for a transaction to get confirmed\n"
            "within nblocks blocks.\n"
            "\nArguments:\n"
            ...
            "blocks have been observed to make an estimate.\n"
            "\nExample:\n"
            + HelpExampleCli("bitcoin_estimatefee", "6")
            );
    ...
    CFeeRate feeRate = bitcoin_mempool.estimateFee(nBlocks);
    if (feeRate == CFeeRate(0))
        return -1.0;
...
Value credits_estimatepriority(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
    ...
    return credits_mempool.estimatePriority(nBlocks);
}

Value bitcoin_estimatepriority(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "bitcoin_estimatepriority nblocks\n"
            "\nEstimates the approximate priority\n"
            "a zero-fee transaction needs to get confirmed\n"
            "within nblocks blocks.\n"
            "\nArguments:\n"
            ...
            "blocks have been observed to make an estimate.\n"
            "\nExample:\n"
            + HelpExampleCli("bitcoin_estimatepriority", "6")
            );
    ...
    return bitcoin_mempool.estimatePriority(nBlocks);
}
