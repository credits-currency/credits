...
public:
    Bitcoin_CBlockAverage() : feeSamples(100), prioritySamples(100) { }
    ...
    bool AreSane(const std::vector<CFeeRate>& vecFee)
    {
        BOOST_FOREACH(CFeeRate fee, vecFee)
        {
            if (fee < CFeeRate(0))
                return false;
            if (fee.GetFee(1000) > Bitcoin_CTransaction::minRelayTxFee.GetFee(1000) * 10000)
                return false;
        }
        return true;
    }
...
class Bitcoin_CMinerPolicyEstimator
{
private:
    // Records observed averages transactions that confirmed within one block, two blocks,
    // three blocks etc.
    std::vector<Bitcoin_CBlockAverage> history;
    ...
    void seenTxConfirm(CFeeRate feeRate, double dPriority, int nBlocksAgo)
    {
        ...
        bool sufficientFee = (feeRate > Bitcoin_CTransaction::minRelayTxFee);
        bool sufficientPriority = Bitcoin_AllowFree(dPriority);
...
public:
    Bitcoin_CMinerPolicyEstimator(int nEntries) : nBestSeenHeight(0)
    {
        history.resize(nEntries);
    }

    void seenBlock(const std::vector<Bitcoin_CTxMemPoolEntry>& entries, int nBlockHeight)
    {
        ...
        // to confirm.
        std::vector<std::vector<const Bitcoin_CTxMemPoolEntry*> > entriesByConfirmations;
        entriesByConfirmations.resize(history.size());
        BOOST_FOREACH(const Bitcoin_CTxMemPoolEntry& entry, entries)
        {
        ...
        {
            std::vector<const Bitcoin_CTxMemPoolEntry*> &e = entriesByConfirmations.at(i);
            ...
            BOOST_FOREACH(const Bitcoin_CTxMemPoolEntry* entry, e)
            {
    ...
    void Write(CAutoFile& fileout) const
    {
        ...
        BOOST_FOREACH(const Bitcoin_CBlockAverage& entry, history)
        {
            entry.Write(fileout);
        }
    }
...
Bitcoin_CTxMemPool::Bitcoin_CTxMemPool()
{
    ...
    minerPolicyEstimator = new Bitcoin_CMinerPolicyEstimator(25);
}
...
void Bitcoin_CTxMemPool::removeForBlock(const std::vector<Bitcoin_CTransaction>& vtx, unsigned int nBlockHeight,
                                std::list<Bitcoin_CTransaction>& conflicts)
{
    LOCK(cs);
    std::vector<Bitcoin_CTxMemPoolEntry> entries;
    ...
    BOOST_FOREACH(const Bitcoin_CTransaction& tx, vtx)
    {
        std::list<Bitcoin_CTransaction> dummy;
...
CFeeRate Bitcoin_CTxMemPool::estimateFee(int nBlocks) const
{
    LOCK(cs);
    return minerPolicyEstimator->estimateFee(nBlocks);
}
double Bitcoin_CTxMemPool::estimatePriority(int nBlocks) const
{
    LOCK(cs);
